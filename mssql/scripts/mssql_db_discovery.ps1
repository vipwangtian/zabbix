...
Get-Service | Where-Object {($_.Name -like 'MSSQL$*' -or $_.Name -like 'MSSQLSERVER') -and $_.Status -eq 'Running'}| ForEach-Object{
    ...
    try{
        ...
        if ($connection -ne $null){
                ...
                $SqlCmd.CommandText = "SELECT @@servicename as inst, name FROM  sysdatabases where name <> 'HADR ÖÖ×Ó²âÊÔ e1b477af-89b7-48bf-893d-3dccd72ece29'"
...
{
    if ($name.inst -eq "MSSQLSERVER")
    {
        $inst = "SQLSERVER"
    }
    else
    {
        $inst = "MSSQL$" + $name.inst
    }
    
    if ($idx -lt $basename.Rows.Count)
    {
        $line= "{ `"{#INST}`" : `"" + $inst + "`", "  + "`"{#DBNAME}`" : `"" + $name.name + "`" }," | convertto-encoding "cp866" "utf-8"
        write-host $line
    }
